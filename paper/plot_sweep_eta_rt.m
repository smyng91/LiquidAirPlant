...
hold on
x = p_r(abs(mdot-mdot_la)<1e-1); y = T_rec_Th_out(abs(mdot-mdot_la)<1e-1);
vq = interp1(x, y, xq, method);
h7 = plot(xq, vq, '-xk','MarkerFaceColor','k');
hold on
legend([h1,h2,h3,h4,h5,h6,h7],'$T_3$','$T_4$','$T_5$','$T_6$','$T_7$','$T_8$','$T_9$','orientation','horizontal','interpreter','latex','FontSize',24,'box','off','location','south')
