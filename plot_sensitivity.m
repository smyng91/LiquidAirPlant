...
gplotmatrix(x,[],group,[colorMap],['o'],1,'off','grpbars',xlabel,xlabel)
hold on
...
%%
nt = 12; % 10 - eta_rt; 11 - eta_II; 12 - COP
ymin = linspace(min(solution(:,nt)),max(solution(:,nt)),n);
group = ones(length(solution(:,nt)),1);
for i=1:1:n - 1
    group(solution(:,nt)>=ymin(i) & solution(:,nt)<=ymin(i+1)) = i+1;
...
% end
pbaspect([1 1 1])
sgtitle('Coefficient of performance ($\mathrm{COP}$)','FontSize',30)
colormap(colorMap)
caxis([ymin(1) ymin(end)])
clb=colorbar;
clb.YTick = ymin;
clb.YTickLabel = (sprintf('%2.3g\n',linspace(ymin(1),ymin(end),length(solution(1,:)))));
colorbar('Position', [.91 0.11  0.01  0.81])
print('paper\mplot_COP','-djpeg','-r400')
