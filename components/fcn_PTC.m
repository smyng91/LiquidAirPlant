function [T, p_out, X_sun, X_dest, eta_c] = fcn_PTC( PTC, AMBIENT )
...
try
    ...
    T_out = PTC.T_in + Qu/(PTC.mdot*cp_HTF);
    T_c = (PTC.A_r*h_r_rc*PTC.T_r+PTC.A_c*(h_r_c0+h_w)*AMBIENT.T0)/(PTC.A_r*h_r_rc+PTC.A_c*(h_r_c0+h_w));
    T_r = (AMBIENT.Gb*PTC.eta_opt*PTC.A_ap-Qu)/(PTC.A_r*U_l)+AMBIENT.T0;
    ...
    s_out = py.CoolProp.CoolProp.PropsSI('Smass','P',p_out,'T',T_out,PTC.FLUID);
    ...
    X_dest_dP = PTC.mdot*AMBIENT.T0*dp/rho_f*log(T_out/PTC.T_in)/(T_out-PTC.T_in);
    X_dest_Q1 = X_sun*PTC.eta_opt-AMBIENT.Gb*PTC.A_ap*PTC.eta_opt*(1-AMBIENT.T0/T_r);
    X_dest_Q2 = PTC.mdot*AMBIENT.T0*(s_out-s_in)-AMBIENT.T0*Qu/T_r;
